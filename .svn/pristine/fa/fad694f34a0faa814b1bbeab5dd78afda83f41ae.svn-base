			...
			String toolNickName ="";	
				...
				if (fromPage != null)
				{
					switch(fromPage)
					{
						case "OpenFile.jsp": 
						{
							toolNickName = request.getParameter("almToolID");
							...
							if (toolNickName != null)
							{
								...
								folderName = toolNickName + "\\AutomationFramework\\src\\test\\resources\\features\\";
								SubFolderSearch subFld = new SubFolderSearch();
						...
						case "CreateFileNewUI.jsp":
						{
							...
							if (toolNickName != null)
							{
									switch(populateList)
									{
										...
										case "release":{
											String relList = tblDAObj.findByTool(toolNickName, "release", null, null);
											response.setContentType("text/plain");
											response.getWriter().write(relList);
											break;
											}
										...
										case "iteration":{
											String itrList = tblDAObj.findByTool(toolNickName, "iteration", releaseName, null);
											response.setContentType("text/plain");
											response.getWriter().write(itrList);
											break;
											}
										case "feature":{
											String featureList = tblDAObj.findByTool(toolNickName, "feature", releaseName, iterationName);
											response.setContentType("text/plain");
							...
							if (releaseName != null)
							{
								...
								{
									case "iteration":{
										String itrList = tblDAObj.findByRelease(releaseName, "iteration", toolNickName);
										response.setContentType("text/plain");
										response.getWriter().write(itrList);
										break;
										}
									case "feature":{
										String featureList = tblDAObj.findByRelease(releaseName, "feature", toolNickName);
										response.setContentType("text/plain");
										response.getWriter().write(featureList);
										break;
										}
						...
						case "PendingApprovals.jsp":
						{
							toolNickName = request.getParameter("toolNickName");
							if (toolNickName != null)
							{
								toolNickName = toolNickName.trim();
								String listPendingApprovals = tblDAObj.listPendingApprovals(toolNickName);
								System.out.println("listPendingApprovals: " + listPendingApprovals);
								response.setContentType("text/plain");
								response.getWriter().write(listPendingApprovals);
							}
							String userStoryName = request.getParameter("userStoryName");
							if (userStoryName != null)
							{
								userStoryName = userStoryName.trim();
								String newStoryDescription = tblDAObj.getNewStoryDescription(userStoryName);
								System.out.println("listPendingApprovals: " + newStoryDescription);
								response.setContentType("text/plain");
								response.getWriter().write(newStoryDescription);
							}
							break;
						}
						default :
