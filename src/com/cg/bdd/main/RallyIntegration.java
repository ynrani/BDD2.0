...
public class RallyIntegration extends Common_Runtime {
	...
	public RallyIntegration(String toolNickName) {
		try {
			populateRallyConnectionStringVariables(toolNickName);
	 ...
	 * @throws SQLException
	 */
	public void populateRallyConnectionStringVariables(String toolNickName) throws ClassNotFoundException, SQLException
	{	
		cmrt=new Common_Runtime();
		cmrt.connectToDatabase();
		String query = "SELECT * from almtool where ToolNickName='" + toolNickName + "'";
	...
	public void fetchScenarios(String userStoryDescription, String userStoryID) {
		...
		for (int i = 0; i < cnt; i++) {
			...
			scenarioName = scenarios[i].substring(beginIndex, endIndex).trim();
			System.out.println("Scenario Name: " + scenarioName);
			scenarioDescription = scenarios[i].substring(endIndex);
			System.out.println("Scenario Description: " + scenarioDescription);
			//int scenarioID = 0;
			int matchingRecordCount = verifyRecordExists("scenarios","ScenarioName", scenarioName);
			if (matchingRecordCount <= 0) {
				...
				insertQuery += " ScenarioName = \"" + scenarioName + "\"";
				insertQuery += "  where UserStoryID = \"" + userStoryID +  "\";";
				System.out.println("Query is " + insertQuery);
				System.out.println("Update Query Output : "
						+ executeInsertUpdateQuery(insertQuery));
			}
	...
	public String[] getScenario(String userStoryDescription) {
		...
		int ind = userStoryDescription.indexOf("Scenario:");
		System.out.println("Index of Scenario "+ind);
		int count = getNoOfOccurrences(userStoryDescription, "Scenario:");
		System.out.println("No of Scenarios: "+ count);
		...
		if (count > 1)
		{
			for (int i=0; i<count;i++)
			{
				...
				System.out.println("Before substring " + tempUserStoryDescription);
				System.out.println((i+1)+")Index of Scenario "+ind);
				if (ind>0){
					substr = tempUserStoryDescription.substring(ind);
					System.out.println("After substring " + substr);
				}
				if (i<count-1)  //Check whether this is the last occurence. If yes we don't need to search for the next occurrence index.
				{
					ind = substr.indexOf("Scenario:", ind+9);
					tempUserStoryDescription=substr;
					System.out.println("New Index of Scenario "+ind);
				}
				
				if (ind == 0) {
					scenario[i] = tempUserStoryDescription;
				} else if (ind > 0){
					scenario[i] = tempUserStoryDescription.substring(0,ind - 1);
