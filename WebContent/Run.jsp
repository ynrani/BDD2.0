...
function CmdAdd_onclick() {
	//	alert("In cmdclick");
		...
		parameter = parameter + "&fromPage=Run.jsp"
		$.ajax({
					url : "a.html?",
					type : "POST",
					data : parameter,
...
function fnOnSelectRadioButton() {
		...
		//alert(path);
		path = path + "\\" + strTool;
		path = path + "\\AutomationFramework\\src\\test\\resources\\features";
	...
	function evalGroup() {
		...
		for (var i = 0; i < group.length; i++) {
			if (group[i].checked) {
				...
				strTool = strTool + "//AutomationFramework//src//test//resources//features//" + selected_value;
				var id = "fId1";
				var parameter = id + "=" + strTool ;
				parameter = parameter + "&fromPage=Run.jsp"
				$.ajax({
	...
	<div class="wrapperForCenterOfthePage">
		...
		<div class="mainAll">
			...
			<div class="container tdm-central"
				...
				<form name="RunFile" action="ExecuteAUTAutomationServlet" method="post"
					...
					<TABLE	style="margin-left: 235px; margin-bottom: 40px; width: 746px; height: 80px;">
						...
						<TR>
								...
								<div class="dropdown">
									<p>
										<font face="Calibri" color="green"></font> <select
											...
											<%
												/* FileUtility txtopr_1 = new FileUtility();
												
												String txtfile = txtopr_1
														.readTxtFile(".\\resources\\Configuration.txt");
												String root = txtfile; */
												String root = cmrt.m_featurePath;
												//System.out.println("Directory "+root);
											//	String rootPath = getServletContext().getRealPath(root);
							...
							<TD><input type="button"
								...
								class="button" value="Search Story" onclick="CmdAdd_onclick()" />
								<!-- <input type="button"
		...
		Selected user story will appear here...</textarea>
					...
					<input type="hidden" id="Folder1" name="Folder1" value="" /> <input
						...
						value="" /> 
						<input id="run" type="submit" class="button"
						value="Run" align="middle" style="margin-right: 1px;" /> <input
